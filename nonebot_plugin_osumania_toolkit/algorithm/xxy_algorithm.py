...
def query_cumsum_vec(q_arr, x, F, f):
    """Vectorised version: returns cumulative sum at arbitrary points q_arr."""
...
# ----- Original stream_booster (restored) -----
...
def compute_anchor(K, key_usage_400, base_corners):
    # Vectorised anchor calculation
    counts = np.stack([key_usage_400[k] for k in range(K)], axis=1)
    counts_sorted = np.sort(counts, axis=1)[:, ::-1]  # descending per row

    nonzero_mask = counts_sorted > 0
    n_nz = nonzero_mask.sum(axis=1)

    # Prepare for walk calculation
    c0 = counts_sorted[:, :-1]
    c1 = counts_sorted[:, 1:]
...
def compute_Pbar(K, T, x, note_seq, LN_rep, anchor, base_corners):
    ...
    for i in range(len(note_seq) - 1):
        ...
        b_val = stream_booster(delta)  # original version
        if delta < 2 * x / 3:
